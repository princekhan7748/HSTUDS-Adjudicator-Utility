...
*   **Tournament Mode:**
    *   **Offline Interconnected Jury Session:** A peer-to-peer network for real-time communication between participants using WebRTC.
    *   **Adjudicator and Participant Roles:** Users can either start a tournament as an "adjudicator" or join as a "participant".
    *   **Master Timer Control:** The adjudicator controls a master timer that is synchronized for all participants.
    *   **Remote Speaker Control:** The adjudicator can remotely mute a participant's microphone.
    *   **Room-based joining:** Participants join a tournament using a unique room ID generated by the adjudicator.
*   **UI/UX:**
    ...
    *   Mode toggle switch for "Normal" vs. "Tournament" mode.
    *   Footer with branding and copyright information.
*   **Navigation:**
    *   A reusable `Navbar` component provides consistent navigation across all pages.
...
## 3. Completed Task: Tournament Mode Implementation

**Objective:** Implement a "Tournament Mode" for offline, interconnected jury sessions.

**Features Implemented:**

*   **UI for Mode Toggling:** Added a switch on the main page to toggle between "Normal Mode" and "Tournament Mode".
*   **Tournament Page (`/tournament`):** Created a new page as the central hub for the tournament.
*   **`useTournament` Hook:** Developed a custom hook to manage the WebRTC connections, participant roles, and signaling.
*   **Adjudicator View:** Designed an interface for adjudicators with a master timer control panel and participant mute controls.
*   **Participant View:** Designed a basic interface for participants that reflects the state of the master timer.
*   **WebRTC Signaling:** Implemented a basic signaling mechanism using the adjudicator as a central point for exchanging WebRTC offers, answers, and ICE candidates.

## 4. Future Development: Serverless Peer-to-Peer File Sharing
